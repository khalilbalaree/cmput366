...
class MonteCarloAgent(BaseAgent):
    ...
    def agent_init(self):
        ...
        self.Q = np.zeros((100,51))
        self.visitCount = np.zeros((100,100))
        self.returns = np.zeros((100,100))
        for s in range(1,100):
            self.policy[s] = np.random.random_integers(1, min(s, 100 - s))                                                                                                           
    ...
    def agent_step(self, reward, state):
        ...
        self.visitCount[int(state[0])][action] += 1
        self.thisEpisode.append([int(state[0]), action])
    ...
    def agent_end(self, reward):
        ...
        """
        # print(self.thisEpisode)
        for SA in self.thisEpisode:
            state = SA[0]
            action = SA[1]
            self.returns[state][action] += reward

        for s in range(1,100):
            for a in range(1, min(s, 100 - s) + 1):
                count = self.visitCount[s][a]
                if count != 0:
                    self.Q[s][a] = self.returns[s][a] / count
                    # print(self.Q[s][a])
